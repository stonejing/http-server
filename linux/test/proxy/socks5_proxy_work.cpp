...
void app_socket_pipe(int fd0, int fd1) {
  ...
  log_message("Connecting two sockets: %d %d", fd0, fd1);
  ...
  while (1) {
    ...
    if (FD_ISSET(fd0, &rd_set)) {
      ...
      if (nread <= 0) {
        log_message("remote server closed connection.");
        break;
      }
    ...
    if (FD_ISSET(fd1, &rd_set)) {
      ...
      if (nread <= 0) {
        log_message("local app closed connection.");
        break;
      }
...
int app_loop() {
  ...
  while (1) {
    ...
    log_message("accpet new connection.");
    int one = 1;
...
void usage(char *app) {
  printf("USAGE: %s [-h][-n PORT][-a AUTHTYPE][-u USERNAME][-p PASSWORD][-l "
         "LOGFILE]\n",
         app);
